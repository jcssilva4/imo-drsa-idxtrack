...
def get_datatable(rets, I, rule, DT_size, R_final, intrLvl, DM_weights, frontier_filter, DT_presentation):
	...
	fit = get_fitness_individuals(R_final, rets, I)
	sample = get_drsa_eval(np.array(R_final), rets, I, DT_presentation)
	sample = np.array(sample)
	F = non_dominated_sort(fit, pref_dir) 
	...
	# get data table
	bestAlternative, DT, unique_sol = write_this_table(rank_idx[:2*DT_size], samples_bestFront_R, sample_fitness, DT_size, DM_weights, DT_presentation, rets, I) 	

	return bestAlternative, DT, unique_sol
...
def write_this_table(rank_idx, R, sample, DT_size, DM_weights, DT_presentation, rets, I):
	...
	unique_sol = False # assume that there is no unique_sol
	for idx in rank_idx:
	...
	# check if unique sol found and if it is inconsistent (unique sol classified as 'good' and 'other'):
	if all(decexample == examples[0] for decexample in examples[:DT_size+1]):
		unique_sol = True
	...
	return best_alternative, DT_examples, unique_sol

# this function computes the Chebyshev utility function for each candidate portfolio
def get_pref_func(R, DM_weights, rets, I):
	...
	for m in range(examples.shape[1]):
		# get the maximum and minimum value for each objective
		f_ref = 0 # reference point to compute the chebyshev utility (meanTE, max(TE))
		if m == 2: # ER
			f_ref = 1
		if m == 3: # max(-ER)
			f_ref = -1
		...
		for exmpl in range(examples.shape[0]):
			f = examples[exmpl,m]
			chebyshev_val_m = DM_weights[m]*np.abs(f - f_ref)
			if pref_func[exmpl] < chebyshev_val_m: # get max(w*|f_m-z_m|)
				pref_func[exmpl] = chebyshev_val_m
	#print(pref_func)
	pref_order = np.argsort(pref_func).tolist()
	#print("pref_func: " + str(pref_func))
	#print("pref_order_before: " + str(pref_order))
	#pref_order.reverse()
	#print("pref_order_after: " + str(pref_order))
...
def get_pref_func_rebal(portfolio, DM_weights):
	...
	for m in range(portfolio.shape[0]):
		...
		f = portfolio[m]
		chebyshev_val_m = DM_weights[m]*np.abs(f - f_ref)
		if pref_func < chebyshev_val_m: # get max(w*|f_m-z_m|)
			pref_func = chebyshev_val_m
	return pref_func
...
def get_best_alternative_nsga2(rets, I, R_final, DM_weights):
	pref_dir = get_pref_dir() # -> pref_dir_m = -1 to minimize and pref_dir_m = 1 to maximize
	# get fitness for all individuals of current R_final
	fit = get_fitness_individuals(R_final, rets, I)
	F = non_dominated_sort(fit, pref_dir) 
	...
	# get fitness of each individual of the population
	for s in F[0]:
		samples_bestFront.append(fit[s])
		samples_bestFront_R.append(R_final[s])
	sample_fitness = np.array(samples_bestFront)
	# now we need to calculate the pref_func of the examples
	pref_order, DT_examples, pref_func = get_pref_func(samples_bestFront_R, DM_weights, rets, I)
	# get the best alternative (choice)
	best_alternative = np.array(samples_bestFront_R[pref_order[0]])
	return best_alternative


def sample_rand_sols(rets, I, DT_size, R_final, DT_presentation):
	...
	return rank_idx, fit, sample_fitness


def write_this_table_examples(R, sample, DT_size, DM_weights, DT_presentation, rets, I, another_filepath):
	R_all = R.copy()
	...
	for obj in examples:
		#print(obj)
		line = '\nportfolio_' + str(pref_order[exmplcounter-1]+1) + ": "
		#print(line)
		for m in obj:
			#print(m)
			line += str(m)
	...
	# open the file to save the current data table
	data_table = open(another_filepath + "currentDT.isf","w")
	# write the header
	data_table.writelines("**ATTRIBUTES\n\n")
	criteria, sense, trash = get_head_DT_presentation(DT_presentation)
	for obj in range(len(criteria)):
